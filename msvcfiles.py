...
def generate_src_list (srcroot, srcdir, filters_src, filter_conds, filter_c, custom_makefile_name):
    makefile_name = 'Makefile.am'
    if custom_makefile_name is not None:
        makefile_name = custom_makefile_name
    print("Reading %s" % os.path.join(srcdir, makefile_name))
    vars = read_vars_from_AM(os.path.join(srcdir, makefile_name),
...
def gen_vs9_project (projname, srcroot, srcdir_name, sources_list):
    vs_file_list_dir = os.path.join (srcroot, 'win32')
    ...
    process_include (os.path.join(srcroot, 'win32', 'vs9', projname + '.vcprojin'),
                     os.path.join(srcroot, 'win32', 'vs9', projname + '.vcproj'),
                     includes = [vs_file_list_dir])

    os.unlink(os.path.join(srcroot, 'win32', projname + '.sourcefiles'))
...
def gen_vs10_project (projname, srcroot, srcdir_name, sources_list):
    ...
    process_include (os.path.join(srcroot, 'win32', 'vs10', projname + '.vcxprojin'),
                     os.path.join(srcroot, 'win32', 'vs10', projname + '.vcxproj'),
                     includes = [vs_file_list_dir])
    process_include (os.path.join(srcroot, 'win32', 'vs10', projname + '.vcxproj.filtersin'),
                     os.path.join(srcroot, 'win32', 'vs10', projname + '.vcxproj.filters'),
                     includes = [vs_file_list_dir])

    os.unlink(os.path.join(srcroot, 'win32', projname + '.vs10.sourcefiles'))
    os.unlink(os.path.join(srcroot, 'win32', projname + '.vs10.sourcefiles.filters'))
        ...
        for file_list, srcdir, dir_name in zip (inst_lists, srcdirs, destdir_names):
            for i in file_list:
                vsinstlist.write ('copy ..\\..\\' +
                                  ...
                                  dir_name +
                                  vsprops_line_ending)
    process_include (os.path.join(srcroot, 'win32', 'vs' + vsver, projname + '-install' + vsprops_file_ext + 'in'),
                     os.path.join(srcroot, 'win32', 'vs' + vsver, projname + '-install' + vsprops_file_ext),
                     includes = [vs_file_list_dir])
...
def generate_nmake_makefiles(srcroot, srcdir, base_name, makefile_name, progs_list):
    file_list_dir = os.path.join (srcroot, 'win32')
